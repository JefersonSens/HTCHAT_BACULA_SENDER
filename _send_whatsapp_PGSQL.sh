...
where JobMedia.JobId = Job.JobId limit 1 ) as Storage, TO_CHAR( Job.StartTime , 'HH24:MI:SS' ) as StartTime, TO_CHAR( Job.EndTime , 'HH24:MI:SS' ) as EndTime,
extract(epoch from  Job.StartTime - Job.EndTime) as Duration, Job.JobStatus,
...
str=`echo -e "$sql_query" | psql -U $DBUSER -d $DBNAME -AF $'\t' |  while read; do sed 's/t/|/g'; done`
